...
cv_train <-
           ...
           MaxNWts = 2000, maxit = 2000)
  {
    ...
    ctrl <- caret::trainControl(method = "cv" , number = folds, classProbs = T, summaryFunction = caret::twoClassSummary)
    ...
    Maxindex <- which.max(nnfit$result$ROC)

    print(ggplot2::ggplot(data = nnfit$result, aes(x = decay, y = ROC, group = size, color = factor(size))) +
            geom_path()+
            geom_point(size = 1.5)+
            geom_point( x = nnfit$result$decay[Maxindex], y = nnfit$result$ROC[Maxindex], color = "red", size = 4)+
            theme_bw() +
            theme(axis.line = element_line(colour = "black"),
                  panel.grid.major = element_blank(),
                  panel.grid.minor = element_blank()) +
            ggtitle(paste0("Max ROC Area = ", format(nnfit$result$ROC[Maxindex], digits = 3), "; Best Decay = ",  format(nnfit$result$decay[Maxindex], digits = 4), "; Best Size = ",  format(nnfit$result$size[Maxindex], digits = 3)))+
            labs(color = "Size", x = "Decay", y = "Area Under ROC")

    )

    output <- list("PropensityScore" = predict(nnfit$finalModel, data), "Performance" = nnfit$result,"BestPerformance" = nnfit$result[Maxindex,],"BestTune" = nnfit$bestTune, "FittedModel" = nnfit)
    return(output)
  }
